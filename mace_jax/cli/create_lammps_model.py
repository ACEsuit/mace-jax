...
def main() -> None:
    ...
    output_path = (
        ...
        else Path(args.model_path).with_suffix('')
    ...
    print('  from mace_jax.calculators.lammps_mliap_mace import create_lammps_mliap_calculator')
    print('  artifact = pickle.load(open(path, \"rb\"))')
    print('  jax_model, variables, _ = convert_model(torch_model, artifact[\"config\"])')
    print('  wrapper = create_lammps_mliap_calculator(jax_model, variables, head=artifact[\"head\"])')
