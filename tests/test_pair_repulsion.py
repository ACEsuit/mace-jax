...
def _energy_from_model(graphdef, params, data) -> float:
    outputs, _ = graphdef.apply(params)(data, compute_force=False, compute_stress=False)
...
def test_pair_repulsion_raises_close_contact_energy():
    ...
    graphdef_no_rep, params_no_rep = nnx.split(model_no_rep)

    model_with_rep = _build_mace_model(pair_repulsion=True)
    graphdef_with_rep, params_with_rep = nnx.split(model_with_rep)

    energy_close_no = _energy_from_model(graphdef_no_rep, params_no_rep, close_data)
    energy_far_no = _energy_from_model(graphdef_no_rep, params_no_rep, far_data)

    energy_close_with = _energy_from_model(
        graphdef_with_rep, params_with_rep, close_data
    )
    energy_far_with = _energy_from_model(graphdef_with_rep, params_with_rep, far_data)
...
def test_pair_repulsion_contributes_to_interaction_energy():
    ...
    out_close_no, _ = graphdef_no_rep.apply(params_no_rep)(
        close_data, compute_force=False, compute_stress=False
    )
    out_far_no, _ = graphdef_no_rep.apply(params_no_rep)(
        far_data, compute_force=False, compute_stress=False
    )
    out_close_rep, _ = graphdef_with_rep.apply(params_with_rep)(
        close_data, compute_force=False, compute_stress=False
    )
    out_far_rep, _ = graphdef_with_rep.apply(params_with_rep)(
        far_data, compute_force=False, compute_stress=False
    )
...
def test_distance_transform_changes_radial_embedding():
    ...
    graphdef_none, params_none = nnx.split(block_none)
    out_none, _ = graphdef_none.apply(params_none)(
        lengths,
        node_attrs,
        edge_index,
        atomic_numbers,
    ...
    graphdef_agn, params_agn = nnx.split(block_agn)
    out_agn, _ = graphdef_agn.apply(params_agn)(
        lengths,
        node_attrs,
        edge_index,
        atomic_numbers,
    )
    if isinstance(out_agn, tuple):
        out_agn = out_agn[0]
